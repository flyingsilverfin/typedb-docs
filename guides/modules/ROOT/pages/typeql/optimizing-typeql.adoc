...
:summary: Learn how TypeDB's optimizer works and how to write efficient TypeQL queries.
:page-toclevels: 2
...
When computing a recursive function, a new table will be created for each combination of arguments the function is called with. Certain formulations of functions are more efficient than others under tabling:
...
`recurse_on_from` is almost unintuitive as most programmers would expect it to suffer from infinite recursion. **However it is the more efficient formulation!**.
