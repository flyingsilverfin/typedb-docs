...
User-defined types, their interfaces, and their subtyping structure, are created and modified through xref:{page-version}@reference::typeql/index.adoc#statements[definition statements] in xref:{page-version}@reference::typeql/index.adoc#queries[schema queries].
...
Stages in TypeQL xref:{page-version}@reference::typeql/index.adoc#queries[data pipeline] will contain xref:{page-version}@reference::typeql/index.adoc#statements[statements] with variables. Any variable in a stage belongs to exactly one of four categories.
