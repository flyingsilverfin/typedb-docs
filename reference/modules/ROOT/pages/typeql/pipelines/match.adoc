= Match stage
:page-aliases: {page-version}@reference::typeql/patterns/matching.adoc
...
* For each input row, the match stage outputs all combinations of data for its xref:{page-version}@reference::typeql/data_model.adoc#modes[output variables] (i.e., those variables that have not already been bound in earlier stages, and which are not xref:{page-version}@reference::typeql/data_model.adoc#modes[internal] to the stage).
