...
Duration values are produced by subtracting temporal values (dates, datetimes, or datetime-tz values). See xref:{page-version}@typeql-reference::expressions/operators.adoc[] for the full operator table.
...
Two `duration` values can be added or subtracted. Each component (months, days, nanoseconds) is added or subtracted independently. Subtraction cannot produce negative results.
