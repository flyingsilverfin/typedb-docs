...
TypeDB is made up of a few simple xref:{page-version}@core_concepts::typedb/index.adoc[database concepts].
By using the https://cloud.typedb.com[fully managed service,window=_blank], or a local xref:{page-version}@home::install/ce.adoc[TypeDB CE server],
...
TypeDB's baseline types are xref:{page-version}@core_concepts::typeql/entities-relations-attributes.adoc[Entities, Relations, and Attributes].
...
You can even create reusable xref:{page-version}@core_concepts::typeql/queries-as-functions.adoc[functions] to capture modular pieces of database logic and invoke them at query time.
...
Combined with xref:{page-version}@core_concepts::typedb/transactions.adoc[transactions], you can be confident of moving between correct states. Even better,
...
Feel free to explore our xref:{page-version}@examples::index.adoc[Examples and use cases] to get a feel for how TypeDB applications can look.

== Next steps

Check out our xref:{page-version}@home::learning_journey.adoc[Learning Journey] for a guided path through the documentation,
or jump into the xref:{page-version}@home::get_started/setup.adoc[Get started guide].