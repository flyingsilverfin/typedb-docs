...
Now we'll use the previously created server to create a database and initialize the schema.
...
Before we continue creating the database schema, let's have a quick look at TypeDB's data model. In TypeDB, data is stored in *types*, which can be either:
...
Let's define a simple database schema: we want to store ``user``'s and their ``username``'s in our database and, in addition, let's also permit ``friendship``'s between users.
