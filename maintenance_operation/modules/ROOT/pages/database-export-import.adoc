...
You can import your databases exported from TypeDB 2.x following xref:{page-version}@reference::typedb-2-vs-3/process.adoc[this guide].
====
...
This is the best tool you can use to migrate a database between two incompatible versions of TypeDB (e.g., a migration xref:{page-version}@reference::typedb-2-vs-3/process.adoc[from TypeDB 2.x to TypeDB 3.x]).

Both operations are performed through the network, but the server and the client can be used on the same host.

Examples below use xref:{page-version}@home::install/console-cli.adoc[TypeDB Console] as the most accessible client for these operations.
Please check the xref:{page-version}@reference::typedb-grpc-drivers/index.adoc[gRPC Drivers API] to explore available programmatic commands.
...
While the database data is being exported, parallel transactions are allowed, but none of them will affect the exported data thanks to TypeDB's xref:{page-version}@core_concepts::typedb/transactions.adoc[transactional nature].
...
2. Follow xref:{page-version}@tools::console.adoc[Console page] to set up a connection.
...
* Ensure that you use xref:{page-version}@tools::console.adoc#version-compatibility[compatible] TypeDB Server and TypeDB Console.
...
Check out xref:{page-version}@reference::typedb-2-vs-3/process.adoc[this page] for a detailed guide.
