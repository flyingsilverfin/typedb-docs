...
Explore the xref:{page-version}@manual::index.adoc[Manual], xref:{page-version}@new_reference::typeql/index.adoc[TypeQL Documentation], and https://github.com/typedb/typedb/releases[release notes] for the full picture.
...
* **Sessions Removed:** Connections to TypeDB now only require opening a `schema`, `write`, or `read` xref:{page-version}@new_core_concepts::typedb/transactions.adoc[transaction].
...
Default credentials, if applicable, can be found in the xref:{page-version}@new_core_concepts::drivers/connections.adoc[Connections page].
* **Updated Query Syntax:**
- xref:{page-version}@new_reference::typeql/schema/undefine.adoc[`undefine`] and xref:{page-version}@new_reference::typeql/pipelines/delete.adoc[`delete`] now explicitly "point" to their removed targets, reducing ambiguity.
- xref:{page-version}@new_reference::typeql/pipelines/fetch.adoc[`fetch`] syntax aligns with JSON output formatting.
- xref:{page-version}@new_reference::typeql/pipelines/match.adoc[`match`] produces visible row-like results itself (similarly to xref:{page-version}@new_reference::typeql/pipelines/insert.adoc[`insert`], etc): no more `get` s!
- All `$vars` start with `$`: no more `?` for value variables!
However, it is still easy to differentiate their declaration with the new xref:{page-version}@new_reference::typeql/statements/let-eq.adoc[`let` keyword].
* **Annotations and Statements Consolidated:** xref:{page-version}@new_reference::typeql/annotations/abstract.adoc[`abstract`] and xref:{page-version}@new_reference::typeql/annotations/regex.adoc[`regex`] are now grouped under xref:{page-version}@new_reference::typeql/annotations/index.adoc[@annotations].
[#_specialize]
...
- As before, use the `as` keyword to xref:{page-version}@new_reference::typeql/statements/relates.adoc[specialize a `relates`].
...
* **Rules and Inference Replaced:** Rules are now replaced by xref:{page-version}@new_reference::typeql/functions/index.adoc[functions], requiring explicit functions execution instead of inference, but offering more abstraction and flexibility.
...
Consider using one of xref:{page-version}@new_reference::typedb-grpc-drivers/index.adoc[the available languages] or xref:{page-version}@new_reference::typedb-http-api.adoc[the HTTP Endpoint].

[#_having_troubles]
== Having troubles?

For troubleshooting tips and guidance, refer to the xref:{page-version}@new_maintenance_operation::troubleshooting/index.adoc[Troubleshooting page].
