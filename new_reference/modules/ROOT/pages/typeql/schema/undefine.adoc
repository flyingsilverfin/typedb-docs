...
* xref:{page-version}@reference::typeql/statements/relates.adoc[`relates ... from` statements] undefine existing relates and role types.
* xref:{page-version}@reference::typeql/statements/relates.adoc[`as ... from relates` statements] undefine existing relates specializations.
* xref:{page-version}@reference::typeql/statements/plays.adoc[`plays ... from` statements] undefine existing roleplayings.
* xref:{page-version}@reference::typeql/statements/owns.adoc[`owns ... from` statements] undefine existing ownerships.
* xref:{page-version}@reference::typeql/statements/value.adoc[`value ... from` statements] undefine existing attribute types' value types.
* xref:{page-version}@reference::typeql/statements/sub.adoc[`sub ... from` statements] undefine existing subtyping connections.
* xref:{page-version}@reference::typeql/annotations/index.adoc[`@annotation ... from` statements] undefine existing annotations.
